...
`inspect_app` and `scaffold_mobile_auth` will be unavailable, and `/export:mobile`
...
It reads the web app's models, JS modules, and routes, and writes the
...
From the export, determine whether `/api/auth/login_token` is registered in the web app.

- Present → mobile auth is ready.
- Absent and at least one screen calls `requireAuth()` → the app needs Bearer token
  endpoints. If the `gova-builder` MCP is connected, tell the developer `/build` will
  call `scaffold_mobile_auth` as its first plan step (it is idempotent). If the MCP is
  not connected, tell them to run `scaffold_mobile_auth` from their gova-monolith
  project in Claude Code before `/build`.
...
> - Web App Path: [path]
> - Exported: [N] models, [N] screens ([N] protected)
